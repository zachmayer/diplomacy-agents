...
class _StubModel:  # noqa: D101
    """Minimal stub mimicking the return value of ``pydantic_ai.models.infer_model``."""

    class _Profile:  # noqa: D106
        supports_json_schema_output = False

    profile = _Profile()


class _StubResult:  # noqa: D101  (simple stub)
    ...
    def usage(self) -> Usage:  # noqa: D401
        return self._usage


class _StubAgent:  # noqa: D101
    """Replaces the real *pydantic_ai* Agent during the test."""

    def __init__(self, *args: object, **kwargs: object) -> None:  # noqa: D401, ANN002, ANN003
        """Ignore all constructor parameters – stub only."""

    async def run(self, _prompt: str) -> _StubResult:  # noqa: D401, ANN202
        """Return a dummy result with fixed usage counts."""
        return _StubResult()


def _dummy_state_view() -> tuple[SimpleNamespace, SimpleNamespace]:  # noqa: D401, ANN202
...
def _infer_model_stub(*_args: object, **_kw: object) -> _StubModel:  # noqa: D401, ANN001, ANN003
    """Return a fresh stub model instance (patched over infer_model)."""
    return _StubModel()


def test_stats_capture(monkeypatch: pytest.MonkeyPatch) -> None:  # noqa: D401, ANN001
    ...
    _ = _dummy_state_view()  # noqa: F841

    asyncio.run(agent._run_llm(prompt="hi", output_type=str))  # type: ignore[arg-type]
